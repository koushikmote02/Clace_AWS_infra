...
resource "aws_iam_role" "ehl" {
  name = "${local.name_prefix}-ehl-benchmark-role"
  ...
  tags = { Name = "${local.name_prefix}-ehl-benchmark-role" }
}

resource "aws_iam_role_policy_attachment" "ehl_cloudwatch" {
  role       = aws_iam_role.ehl.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

resource "aws_iam_role_policy_attachment" "ehl_ssm" {
  role       = aws_iam_role.ehl.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

resource "aws_iam_instance_profile" "ehl" {
  name = "${local.name_prefix}-ehl-benchmark-profile"
  role = aws_iam_role.ehl.name
}
...
resource "aws_instance" "ehl" {
  ...
  iam_instance_profile        = aws_iam_instance_profile.ehl.name
